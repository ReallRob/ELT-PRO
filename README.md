...
- 用户代码能力：公式计算支持代码模式；代码块支持无输入、多 DataFrame、Workbook、全局函数、受限 import 和多输出。
...
- 全局函数：代码编辑窗口分为“当前代码”和“全局函数”，全局函数可被所有代码块复用。
...
全局函数内部可以返回普通值；但最终输出给工作流的 `return` / `result` 必须是 DataFrame、Workbook、Worksheet 或它们组成的字典/列表。

代码块会序列化 DataFrame 到子进程，大数据量场景可能有明显耗时和内存开销。接入 Workbook 时，为了保持同一个模板主线对象，会在当前进程执行。
...
2. 在 PyInstaller 命令或 `.spec` 中增加对应 `--collect-all 包名`。
3. 更新代码块面板提示和 `CODE_BLOCK_TASKS.md`。
...
- Workbook 输入在代码块里是引用传递，适合模板主线，但代码块内部修改会影响后续节点。
